    ...
    effects: {
      *init(_, { put, select }) {
        ...
        if (isCreateLike(pathname)) {
          ...
          // 创建的情况下从URL中带参数进入
          if (queryObj) {
            if ('todoId' in queryObj) {
              yield put({
                type: 'updateFormInfo',
                payload: { todoId: queryObj.todoId },
              });
            }
          }
      ...
      *{{getOtherList}}({ payload }, { call, put, select }) {
        const result = yield call(services.{{getOtherList}}, payload);
        if (result.success) {
          yield put({
            type: 'updateState',
      ...
      *{{getDetail}}({ payload }, { call, put, select }) {
        if (payload) {
          const result = yield call(services.{{getDetail}}, payload);
          if (result.success) {
      ...
      *save({ payload }, { call, put, select }) {
        const { formInfo, mediaFilterInfo } = yield select(state => _.get(state, '{{appNamespace}}'));
