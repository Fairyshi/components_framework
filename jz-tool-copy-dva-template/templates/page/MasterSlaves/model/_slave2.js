...
export default {
  ...
  effects: {
    *initSlave2({ payload }, { select, call, put }) {
      const { fidAdapter } = yield select(state => _.get(state, '{{appNamespace}}'));
      yield put({ type: 'resetSlave2', payload })
    ...
    // 保存基本信息
    *saveSlave2(_, { call, put, select }) {
      const { slave2, fidAdapter } = yield select(state => _.get(state, '{{appNamespace}}'));
      const { todo } = outApiAdapter4Save(slave2, fidAdapter);
    ...
    *delSlave2Item({ payload }, { call, put, select }) {
      const { fidAdapter } = yield select(state => _.get(state, '{{appNamespace}}'));
    ...
    *replaceSlave2Item ({ payload }, { call, put, select }) {
      const {  } = payload;
      const oldInfo = yield select(state => _.get(state, `{{appNamespace}}.todo`));
  ...
  reducers: {
    updateSlave2: (state, { payload }) => (
      update.$merge(state, 'slave2', payload)
    ),
    updateSlave2FormInfo: (state, { payload }) => (
      update.$merge(state, 'slave2.formInfo', payload)
    ),
    updateSlave2ErrorInfo: (state, { payload }) => (
      update.$merge(state, 'slave2.errorInfo', payload)
    ),
    delSlave2Item(state, { payload }) {
      const slave2 = _.get(state, 'slave2');
